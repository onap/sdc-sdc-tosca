...
package org.onap.sdc.tosca.parser.api.parameters;
...
import org.onap.sdc.tosca.parser.api.DataEntity;
...
import org.onap.sdc.tosca.parser.enums.ToscaElementNames;
...
public class Input {
    ...
    public void parseAnnotations() {
        if (schema.getSchema() != null) {
            LinkedHashMap<String, Object> annotations = (LinkedHashMap<String, Object>) schema.getSchema().get(
                ToscaElementNames.ANNOTATIONS.getName());
            if (annotations != null) {
        ...
        for (String key : schema.getSchema().keySet()) {
            ...
            if (!bFound) {
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE214", String.format(
