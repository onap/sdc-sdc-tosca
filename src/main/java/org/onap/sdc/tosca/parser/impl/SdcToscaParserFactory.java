...
public class SdcToscaParserFactory {
    ...
    private void handleErrorsByTypes(String csarPath, String cSarConformanceLevel) throws JToscaException {
        ...
        for (JToscaValidationIssue toscaValidationIssue : ThreadLocalsHolder.getCollector().getValidationIssues()
            .values()) {
            List<JToscaValidationIssueInfo> issueInfos = configurationManager.getJtoscaValidationIssueConfiguration()
                .getValidationIssues().get(toscaValidationIssue.getCode());
            if (issueInfos != null && !issueInfos.isEmpty()) {
                ...
                issueInfo = issueInfos.stream()
                    ...
                    .max((i1, i2) -> GeneralUtility
                        .conformanceLevelCompare(i1.getSinceCsarConformanceLevel(), i2.getSinceCsarConformanceLevel()))
                    .orElse(null);
    ...
    private void logErrors(String inputPath) throws JToscaException {
        ...
        if (warningsCount > 0) {
            ...
            log.warn("ToscaTemplate - verifyTemplate - {} Parsing Warning{} occurred...", warningsCount,
                (warningsCount > 1 ? "s" : ""));
        ...
        if (criticalsCount > 0) {
            log.error(
                "####################################################################################################");
            log.error("ToscaTemplate - verifyTemplate - {} Parsing Critical{} occurred...", criticalsCount,
                (criticalsCount > 1 ? "s" : ""));
            ...
            throw new JToscaException(
                String.format("CSAR Validation Failed. CSAR name - {}. Please check logs for details.", inputPath),
    ...
    private void throwConformanceLevelException(String minVersion) throws SdcToscaParserException {
        ErrorInfo errorInfo = configurationManager.getErrorConfiguration()
            .getErrorInfo(SdcToscaParserErrors.CONFORMANCE_LEVEL_ERROR.toString());
    ...
    private void throwSdcToscaParserException(JToscaException e) throws SdcToscaParserException {
        ErrorInfo errorInfo = configurationManager.getErrorConfiguration().getErrorInfo(
            SdcToscaParserErrors.getSdcErrorByJToscaError(JToscaErrorCodes.getByCode(e.getCode())).toString());
