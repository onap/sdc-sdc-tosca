...
//	    }
        ...
        if (!bFound) {
            ...
            for (int argumentNumber = 1; argumentNumber < args.size(); argumentNumber++) {
                String dataTypeName = "";
                bFound = false;
                if (INDEX.equals(args.get(argumentNumber).toString()) || (args
                    .get(argumentNumber) instanceof Integer)) {
                    bFound = true;
                } else {
                    ...
                    //check property name
                    LinkedHashMap<String, Object> dataType = (LinkedHashMap<String, Object>) toscaTpl.getCustomDefs()
                        .get(dataTypeName);
                    if (dataType != null) {
                ...
                if (!bFound) {
                    ThreadLocalsHolder.getCollector()
                        .appendValidationIssue(new JToscaValidationIssue("JE282", String.format(
    ...
    public Object result() {
        if (toscaTpl.getParsedParams() != null &&
            ...
            if (value instanceof ArrayList) {
                if (args.size() == 2 && args.get(1) instanceof Integer && ((ArrayList) value).size() > (Integer) args
                    .get(1)) {
