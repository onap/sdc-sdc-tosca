...
/**
 * This class describes a node template instance containing an entity searched and retrieved by SDC Tosca Parser API It
 * is used as the API input parameter. See the {@link org.onap.sdc.tosca.parser.api.ISdcCsarHelper}
 */
public class TopologyTemplateQuery {
    ...
    public Boolean isMatchingSearchCriteria(NodeTemplate nodeTemplate) {
        boolean isMatched =
            Objects.nonNull(nodeTemplate.getMetaData()) && isSearchedTemplate(nodeTemplate.getMetaData());
        if (logger.isDebugEnabled()) {
            logger.debug("Node template {} is{} matching search criteria", nodeTemplate.getName(),
                isMatched ? "" : " not");
        }
        return isMatched;
    }
    ...
    public static class TopologyTemplateQueryBuilder {
        ...
        private TopologyTemplateQueryBuilder(SdcTypes sdcType) {
