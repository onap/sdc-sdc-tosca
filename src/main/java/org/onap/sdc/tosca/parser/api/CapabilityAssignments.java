...
public class CapabilityAssignments {
    ...
    public CapabilityAssignments(Map<String, CapabilityAssignment> capabilityAssignments) {
        this.capabilityAssignments =
            capabilityAssignments != null ? new HashMap<>(capabilityAssignments) : new HashMap<>();
    }

    /**
     * Get all capability assignments for node template.<br> This object can be either the original one, holding all
     * capability assignments for this node template,or a filtered one, holding a filtered subset.<br>
     *
     * @return list of capability assignments for the node template. <br> If there are no capability assignments, empty
     * list is returned.
     */
     ...
     * @return CapabilityAssignments object, containing capability assignments of this type.<br> If no such found,
     * filtering will result in an empty collection.
     */
    public CapabilityAssignments getCapabilitiesByType(String type) {
        Map<String, CapabilityAssignment> capabilityAssignmentsMap = capabilityAssignments.entrySet().stream()
            .filter(cap -> cap.getValue().getDefinition().getType().equals(type))
