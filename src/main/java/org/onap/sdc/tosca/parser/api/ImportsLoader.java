...
import org.onap.sdc.tosca.parser.elements.TypeValidation;
import org.onap.sdc.tosca.parser.utils.ThreadLocalsHolder;
import org.onap.sdc.tosca.parser.utils.UrlUtils;
...
public class ImportsLoader {
    ...
    public ImportsLoader(ArrayList<Object> _importslist,
        ...
        if ((_path == null || _path.isEmpty()) && tpl == null) {
            ...
            ThreadLocalsHolder.getCollector().appendValidationIssue(
                new JToscaValidationIssue("JE184", "ValidationError: Input tosca template is not provided"));
        }
            ...
            if (importDef instanceof LinkedHashMap) {
                for (Map.Entry<String, Object> me : ((LinkedHashMap<String, Object>) importDef).entrySet()) {
                    ...
                    if (importNames.contains(importName)) {
                        ...
                        //log.error(msg)
                        ThreadLocalsHolder
                            .getCollector().appendValidationIssue(new JToscaValidationIssue("JE186", String.format(
    ...
    /**
     * This method is used to get consolidated custom definitions by passing custom Types from each import. The
     * resultant collection is then passed back which contains all import definitions
     *
        ...
        for (String key : importUri.keySet()) {
            ...
            if (!bFound) {
                ...
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE188", String.format(
    	...
    	*/
        ...
        if (importUriDef instanceof LinkedHashMap) {
            ...
            if (repository != null) {
                if (!repositories.keySet().contains(repository)) {
                    ThreadLocalsHolder.getCollector()
                        .appendValidationIssue(new JToscaValidationIssue("JE189", String.format(
        ...
        if (UrlUtils.validateUrl(fileName)) {
            try (InputStream input = new URL(fileName).openStream();) {
                ...
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE191", String.format(
                        "ImportError: \"%s\" loading YAML import from \"%s\"", e.getClass().getSimpleName(),
                        fileName)));
                al[0] = al[1] = null;
                return al;
            }
            ...
            if (path != null && !path.isEmpty()) {
                if (UrlUtils.validateUrl(path)) {
                    ...
                    if (fp.isAbsolute()) {
                        String msg = String.format(
                            ...
                            fileName, path);
                        ThreadLocalsHolder.getCollector()
                            .appendValidationIssue(new JToscaValidationIssue("JE192", msg));
                        al[0] = al[1] = null;
                        return al;
                    }
                    ...
                    if (fp.isFile()) {
                        ...
                        if (fn.isFile()) {
                            ...
                            String fullPath =
                                Paths.get(path).toAbsolutePath().getParent().toString() + File.separator + fileName;
                            ...
                            if (ffp.isFile()) {
                                ...
                                if (!filePath.isEmpty() && dirPath.endsWith(filePath)) {
                                    ...
                                    if (!fit.isFile()) {
                                        ...
                                        ThreadLocalsHolder
                                            .getCollector()
                                            .appendValidationIssue(new JToscaValidationIssue("JE193", msg));
                ...
                if (fn.isAbsolute() && fn.isFile()) {
                    ...
                    ThreadLocalsHolder
                        .getCollector()
                        .appendValidationIssue(new JToscaValidationIssue("JE194", "ImportError: " + msg));
                    al[0] = al[1] = null;
                    return al;
                }
            }
            ...
            if (importTemplate == null || importTemplate.isEmpty()) {
                ...
                //          {'name': import_uri_def})
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE195", String.format(
            ...
            if (!aFile) {
                log.error(
                    "ImportsLoader - _loadImportTemplate - Error!! Expected a file. importUriDef = {}, importTemplate = {}",
                    importUriDef, importTemplate);
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE196", String.format(
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(importTemplate));) {
                ...
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE197", String.format(
                        "ImportError: Failed to load YAML from \"%s\"" + e, importName)));
                al[0] = al[1] = null;
                return al;
            } catch (Exception e) {
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE198", String.format(
        ...
        if (repository != null && !repository.isEmpty()) {
            ...
            if (fullUrl.isEmpty()) {
                ...
                ThreadLocalsHolder
                    .getCollector().appendValidationIssue(new JToscaValidationIssue("JE200", "ImportError: " + msg));
                al[0] = al[1] = null;
                return al;
            }
        }
        if (UrlUtils.validateUrl(fullUrl)) {
            try (InputStream input = new URL(fullUrl).openStream();) {
                ...
                ThreadLocalsHolder.getCollector()
                    .appendValidationIssue(new JToscaValidationIssue("JE201", String.format(
            ...
            String msg = String.format(
                ...
                repoUrl, importName);
            ThreadLocalsHolder.getCollector()
                .appendValidationIssue(new JToscaValidationIssue("JE202", "ImportError: " + msg));
        }
