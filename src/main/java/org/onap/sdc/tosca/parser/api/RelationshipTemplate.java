...
public class RelationshipTemplate extends EntityTemplate {
    ...
    public ArrayList<Property> _createRelationshipProperties() {
        ...
        if (relationship == null) {
            for (Object val : entityTpl.values()) {
                if (val instanceof LinkedHashMap) {
                    relationship = (LinkedHashMap<String, Object>) ((LinkedHashMap<String, Object>) val)
                        .get("relationship");
                    break;
                }
            }
        ...
        if (relationship != null) {
            properties = (LinkedHashMap<String, Object>) ((EntityType) typeDefinition)
                .getValue(PROPERTIES, relationship, false);
        }
        if (properties == null) {
        ...
        for (PropertyDef p : pds) {
            if (p.getDefault() != null && properties.get(p.getName()) == null) {
                Property prop = new Property(p.getName(), (LinkedHashMap<String, Object>) p.getDefault(), p.getSchema(),
                    customDef);
                props.add(prop);
            }
        }
