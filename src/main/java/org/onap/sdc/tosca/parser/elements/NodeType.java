...
public class NodeType extends StatefulEntityType {
    ...
    private static final String SECTIONS[] = {
        DERIVED_FROM, METADATA, PROPERTIES, VERSION, DESCRIPTION, ATTRIBUTES, REQUIREMENTS, CAPABILITIES, INTERFACES,
        ARTIFACTS
    };
            ...
            for (LinkedHashMap<String, Object> require : requires) {
                    ...
                    if (req.get("relationship") != null) {
                        ...
                        if (nodeType != null) {
                            ...
                            if (nodeType != null) {
                                ...
                                ThreadLocalsHolder
                                    .getCollector()
                                    .appendValidationIssue(new JToscaValidationIssue("JE11", String.format(
                                        "NodeTypeRequirementForCapabilityUnfulfilled: Node type: \"%s\" with requrement \"%s\" for node type with capability type \"%s\" is not found\\unfulfilled",
                                        this.ntype, key, captype)));
                            }
                            if (getRelation != null) {
                ...
                if (relation == null || nodeType == null) {
                    ThreadLocalsHolder.getCollector()
                        .appendValidationIssue(new JToscaValidationIssue("JE11", String.format(
                            "NodeTypeForRelationUnfulfilled: Node type \"%s\" - relationship type \"%s\" is unfulfilled",
                            this.ntype, relation)));
                } else {
    ...
    private void _validateKeys() {
        if (defs != null) {
            for (String key : defs.keySet()) {
                ...
                if (!bFound) {
                    ThreadLocalsHolder.getCollector()
                        .appendValidationIssue(new JToscaValidationIssue("JE124", String.format(
