...
package org.onap.sdc.tosca.parser.elements.constraints;
    ...
    public Pattern(String name, String type, Object c) {
        ...
        if (!validTypes.contains(constraintValue.getClass().getSimpleName())) {
            ThreadLocalsHolder.getCollector().appendValidationIssue(
                new JToscaValidationIssue("JE114", "InvalidSchemaError: The property \"pattern\" expects a string"));
        }
    }
    ...
    protected boolean isValid(Object value) {
        try {
            if (!(value instanceof String)) {
                ThreadLocalsHolder.getCollector().appendValidationIssue(new JToscaValidationIssue("JE115",
                    String.format("ValueError: Input value \"%s\" to \"pattern\" property \"%s\" must be a string",
            ...
            ThreadLocalsHolder.getCollector().appendValidationIssue(new JToscaValidationIssue("JE116",
                String.format("ValueError: Invalid regex \"%s\" in \"pattern\" property \"%s\"",
