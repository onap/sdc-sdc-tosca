...
public class GetEntityPortMirroringTest {
    ...
    public void getServiceConfigurations() {
        ...
        assertEquals("54308d8b-21ca-40a1-bd3e-efde64791605",
            entities.get(0).getMetadata().getValue(SdcPropertyNames.PROPERTY_NAME_CUSTOMIZATIONUUID));
        assertEquals("Port Mirroring Configuration",
    ...
    public void getServiceProxyOnService() {
        ...
        assertTrue("vEPDG-5A-FN SVC Service Proxy"
            .equals(entities.get(0).getMetadata().getValue(SdcPropertyNames.PROPERTY_NAME_NAME)) ||
            "vEPDG-5A-FN SVC Service Proxy"
                .equals(entities.get(1).getMetadata().getValue(SdcPropertyNames.PROPERTY_NAME_NAME)));
        assertTrue("Radcom FN vLBA SVC 11_3_3 vEPDG Service Proxy"
            .equals(entities.get(0).getMetadata().getValue(SdcPropertyNames.PROPERTY_NAME_NAME)) ||
            "Radcom FN vLBA SVC 11_3_3 vEPDG Service Proxy"
                .equals(entities.get(1).getMetadata().getValue(SdcPropertyNames.PROPERTY_NAME_NAME)));
    ...
    public void getCapabilityForCOnfigurationRequirementOnService() {
        ...
        assertEquals(cap0,
            reqMap.stream().filter(f -> f.getName().equals("source")).findAny().get().getCapabilityName());
        assertEquals(cap1,
            reqMap.stream().filter(f -> f.getName().equals("collector")).findAny().get().getCapabilityName());
    ...
    private boolean isCapabilityFound(IEntityDetails nodeDetails, String capName) {
        ...
        CapabilityAssignment capabilityAssignment = assignments.stream()
            .filter(f -> f.getDefinition().getName().equals(capName)).findAny().orElse(null);
