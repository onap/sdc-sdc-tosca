...
public class GetValidationIssues {
    ...
    public static void main(String[] args) {
        ...
        File jtoscaFiles = new File(args[0] + "\\jtosca\\src\\main\\java\\org\\onap\\sdc\\tosca\\parser\\api");

        try {
            printFiles(jtoscaFiles);
            fileWriter = new CSVWriter(
                new FileWriter(args[1] + "\\JToscaValidationIssues_" + System.currentTimeMillis() + ".csv"), '\t');
                    ...
                    try {
                        ...
                        while (scanner.hasNextLine()) {
                            ...
                            if (!line.trim().startsWith("//") && !line.trim().startsWith("#") && line
                                .contains("ThreadLocalsHolder.getCollector().appendValidationIssue")) {
